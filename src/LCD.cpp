...
#define swap_int16_t(a, b) {int16_t c = b; b = a; a = c;}
...
LCD::LCD()
{
	...
	pinMode(LCD_LITE_PIN, OUTPUT);
	
	digitalWriteFast(LCD_CS_PIN, HIGH);
	digitalWriteFast(LCD_CD_PIN, HIGH);
	digitalWriteFast(LCD_WR_PIN, HIGH);
	digitalWriteFast(LCD_RD_PIN, HIGH);
...
void LCD::reset(void) 
{
	...
	if (LCD_RST_PIN)
	{
		digitalWriteFast(LCD_RST_PIN, LOW);
		delay(2);
